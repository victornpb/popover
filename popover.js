...
function Popover(target, options){
    ...
    target.addEventListener(options.trigger, function(e){
        
        if(!target.nextSibling || !target.nextSibling.classList || !target.nextSibling.classList.contains('popover')){
            
            //create popover
            var title=typeof options.title=='function'?options.title.call(target, target):options.title;
            var content=typeof options.content=='function'?options.content.call(target, target):options.content;
            
            options.popover = tag('div', {class: 'popover '+( target.getAttribute('data-placement') )},
                              //tag('div', {class:'arrow'}),
